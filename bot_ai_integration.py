...
# AI Agent instances (one per project)
AI_AGENT_INSTANCES = {}
AI_AGENT_TASKS = {}

async def handle_ai_command(event, command_parts: List[str]):
...
async def handle_learn_strategy(event, args: List[str]):
    ...
    try:
        ...
        if project_id not in AI_AGENT_INSTANCES:
            AI_AGENT_INSTANCES[project_id] = await create_simple_ai_agent(project_id)
        
        agent = AI_AGENT_INSTANCES[project_id]
...
async def handle_create_content(event, args: List[str]):
    ...
    if task_key in AI_AGENT_TASKS and not AI_AGENT_TASKS[task_key].done():
    ...
    task = asyncio.create_task(
        run_ai_content_creation(event, user_id, project_id, topic, strategy_name)
    )
    AI_AGENT_TASKS[task_key] = task

async def run_ai_content_creation(event, user_id: int, project_id: str, topic: str, strategy_name: str):
...
"""
        ...
        # Clean up task
        task_key = f"{user_id}_{project_id}"
        if task_key in AI_AGENT_TASKS:
            del AI_AGENT_TASKS[task_key]
